...
`yolfi_agent_register` は公開エージェント登録エンドポイントを呼び出し、`YOLFI_API_KEY` は不要です。非公開の `yolfi-api` ツールには、登録後に返されるAPIキーが必要です。`yolfi-knowledge` リソースはキーがまだない段階でエージェントが連携手順を理解するのに役立ちます。
