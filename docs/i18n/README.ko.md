...
기본이 아닌 Yolfi 환경을 대상으로 하지 않는 한 `YOLFI_API_BASE_URL`과 `YOLFI_PAY_BASE_URL`은 선택 사항입니다.
...
`yolfi-api` 도구는 Yolfi API를 호출하며 `YOLFI_API_KEY`가 필요합니다. `yolfi-knowledge` 리소스는 키가 생기기 전에도 에이전트가 연동 경로를 이해하도록 돕습니다.
...
| --- | --- | --- |
| Yolfi 작업 공간 등록 | `POST /api/auth/agent/register` | public |
