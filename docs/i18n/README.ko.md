...
비공개 명령은 명시적인 Yolfi API 키 또는 이 기기에 안전하게 저장된 에이전트 인증 정보를 사용합니다:
...
신규 사용자는 이메일과 프로젝트 이름을 확인한 뒤 에이전트가 이메일 확인 등록을 시작할 수 있습니다:
...
이메일은 신규여야 하며 기존 계정은 OAuth 또는 브라우저 설정을 사용합니다. 첫 호출은 확인 링크를 보내고 대기 중인 check-in token을 로컬 설정에 안전하게 저장합니다. 소유자가 링크를 연 뒤 같은 명령을 다시 실행하세요. 그러면 패키지가 한 번만 전달되는 에이전트 인증 정보를 저장하고 CLI와 MCP 출력에서 전체 키를 제거합니다.
...
`yolfi_agent_register`는 공개 에이전트 등록 엔드포인트를 호출하며 `YOLFI_API_KEY`가 필요하지 않습니다. 확인 링크를 연 뒤 같은 도구를 다시 호출하면 한 번만 전달되는 API 키를 로컬에 저장합니다. 비공개 `yolfi-api` 도구는 이 키가 필요합니다. `yolfi-knowledge` 리소스는 키가 생기기 전에도 에이전트가 연동 경로를 이해하도록 돕습니다.
...
Webhook 엔드포인트를 생성하거나 시크릿을 교체하면 CLI는 한 번만 발급되는 서명 시크릿을 보호된 로컬 Yolfi 설정에 저장하고 CLI 또는 MCP 출력에 평문으로 반환하지 않습니다. 저장된 시크릿을 사용하려면 `endpointId`를 `yolfi_webhooks_verify`에 전달하세요. CI 또는 시크릿 관리자 환경에서는 명시적으로 관리하는 `YOLFI_WEBHOOK_SECRET`을 대신 제공할 수 있습니다.
...
| 조직 및 정산 지갑 설정 | `PUT /api/private/organization/current` | bearer API key |
| Webhook 엔드포인트 생성 | `POST /api/private/organization/webhook-endpoints` | bearer API key |
...
- Webhook 서명에는 각 엔드포인트의 보호된 시크릿을 사용하며, 조직 API 키를 서명 시크릿으로 사용하지 않습니다.
- 에이전트 등록은 확인된 check-in에서 API 키를 한 번만 반환하며, 로컬 패키지가 이를 안전하게 저장하고 출력에서 제거합니다.
