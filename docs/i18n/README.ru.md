...
`yolfi_agent_register` вызывает публичный endpoint регистрации агента и не требует `YOLFI_API_KEY`. Закрытые инструменты `yolfi-api` требуют API-ключ, который вернется после регистрации. Ресурсы `yolfi-knowledge` помогают агенту понять путь интеграции до появления ключа.
...
- У каждого Webhook endpoint свой signing secret, который возвращается только при создании endpoint или ротации. Для проверки используйте `--secret` или `YOLFI_WEBHOOK_SECRET`; API-ключ организации не используется для подписи.
