...
Закрытые команды используют API-ключ организации Yolfi:
...
`YOLFI_API_BASE_URL` и `YOLFI_PAY_BASE_URL` необязательны, если вам не нужна нестандартная среда Yolfi.

Если в целевом приложении еще нет `YOLFI_API_KEY`, агент может зарегистрировать рабочее пространство через endpoint регистрации агента:
...
Возвращенный API-ключ показывается один раз. Храните его в игнорируемом env-файле, секрете деплоя или менеджере секретов. Не выводите полный ключ в логи, не коммитьте его и не записывайте в README целевого проекта.
...
Инструменты `yolfi-api` вызывают API Yolfi и требуют `YOLFI_API_KEY`. Ресурсы `yolfi-knowledge` помогают агенту понять путь интеграции до появления ключа.
...
| --- | --- | --- |
| Зарегистрировать рабочее пространство Yolfi | `POST /api/auth/agent/register` | public |
| Проверить аккаунт | `GET /api/private/organization/current` | bearer API key |
| Настроить организацию, Webhook и кошельки для расчетов | `PUT /api/private/organization/current` | bearer API key |
...
- Подпись Webhook-событий использует текущий контракт подписи Yolfi. Если позже Yolfi отделит Webhook-секреты от API-ключей организации, пакет покажет новый путь настройки секрета.
- Регистрация агента возвращает API-ключ один раз. Агенты должны хранить его в игнорируемом env-файле, секрете деплоя или менеджере секретов.
