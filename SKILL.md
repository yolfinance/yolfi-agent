...
- Analytics routing uses one metadata key: `website_id`. Provision Talivia analytics with adapter `NONE` and `metadataFilters: { "website_id": "<websiteId>" }`; do not introduce consumer-prefixed aliases.
